...
function printModalSlip() {
  ...
  doc.write(`
    ...
    <html>
      <head>
        <title>NDBT Slip #${activeSlipTrip.value?.lr_no || ''}</title>
        <meta charset="utf-8" />
...
async function downloadModalSlip() {
  ...
  try {
    // Primary: Download vector PDF with current modal data and selected signatory
    await api.downloadCustomSlipPdf(modalSlipData.value)
  } catch (backendErr) {
